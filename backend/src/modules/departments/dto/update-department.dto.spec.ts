...
/**
 * Only the differences from `CreateDepartmentDto` are asserted here — the
 * shared constraints are covered by that spec, because both DTOs apply the very
 * same decorators.
 */
describe('UpdateDepartmentDto', () => {
  ...
  const metadata: ArgumentMetadata = {
    type: 'body',
    metatype: UpdateDepartmentDto,
  };

  const validate = (body: unknown): Promise<UpdateDepartmentDto> =>
    pipe.transform(body, metadata) as Promise<UpdateDepartmentDto>;
