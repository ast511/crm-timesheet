 ...
 * message beside the code — and the rest are the names the two tiers are keyed
 * and configured by.
 */
 ...
 *
 * `POST /auth/refresh` carries a token and no address, so its strict-tier bucket
 * is keyed on the address alone. A fixed placeholder rather than an empty string
 * keys it explicitly, so the key never ends in a dangling separator that a real
 * value could collide with.
 */
