...
export class ReportingService {
   ...
   * query. It is behind the same access check as the reports themselves — the
   * menu is part of the administrative area, and listing documents somebody may
   * not generate would only produce five buttons that all answer `403`.
   */
  listReports(user: CurrentUser): typeof REPORT_DEFINITIONS {
    assertReportingAccess(user);
...
/**
 * Refuses a caller who does not administer the system.
 *
 * **A domain rule in the service, not a guard, and not the permission engine.**
 * Feature 029 built the permission catalog and enforces none of it, because
 * enforcement needs authentication first — every caller here is still whoever
 * they claim to be in a header. Feature 030 followed that and so does this: no
 * `@RequirePermission()`, no guard, no wiring into the permission system.
 * `PermissionResource.REPORTS` is seeded and waiting for the feature that
 * enforces it, and when authentication lands this check stays exactly as it is
 * with a guard added in front of it.
 *
 * The rule itself is not authorization arriving early. A report is a company-wide
 * document: the attendance sheet lists everybody's absences and the hour
 * matrices state what each colleague earned their month on. "These are
 * administrative documents" is what they *are*, in the same way that "an
 * administrator reviews a timesheet somebody else filled in" is what a timesheet
 * is — it would be true under any permission system.
 *
 ...
 */
export function assertReportingAccess(user: CurrentUser): void {
  if (!isAdministrativeRole(user.role)) {
    throw new ForbiddenException(
      'Reports may only be generated by an administrative role (SUPERADMIN, ADMIN or HR)',
    );
  }
}
