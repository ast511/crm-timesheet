...
export interface PublicHolidayCreateButtonProps {
  variant?: ComponentProps<typeof Button>['variant'];
  size?: ComponentProps<typeof Button>['size'];
}

/**
 * "Adaugă sărbătoare" — the button and the dialog it opens, as one piece.
 *
 ...
 *
 * Gated on `PUBLIC_HOLIDAYS.CREATE` through `useCan` rather than `<Can>`,
 * because the gate covers the dialog as well as the button — and there is no
 * fallback: a control somebody cannot use is simply not there, which is the
 * ordinary case.
 */
export const PublicHolidayCreateButton = ({
  variant,
  size,
}: PublicHolidayCreateButtonProps) => {
  const { t } = useTranslation();
  const [isOpen, setIsOpen] = useState(false);
  const canCreate = useCan({ permission: 'PUBLIC_HOLIDAYS.CREATE' });
  ...
  return (
    <>
      ...
      <PublicHolidayFormDialog open={isOpen} onOpenChange={setIsOpen} />
    </>
  );
};
